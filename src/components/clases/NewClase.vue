    ...
    <div class="container">
        <div class="col-12 text-center">
            <div class="alert alert-success" role="alert">
                <h1 class="mb-0" style="color: green;">Nueva Clase</h1>
            </div>
        </div>
        <p></p>
        <p></p>
    ...
    data() {
        return {
            clase: {
                ...
                hora_inicio: '',
                duracion: 0,
            }, instalaciones: [], id: '0', entrenadores: [], id: '0'
        }
    },
    methods: {
        cancelar() {
        ...
        async saveClase() {
            console.log(`${this.clase.id} ${this.clase.number} ${this.clase.date} ${this.clase.customer_id} ${this.clase.pay_mode_id}`)
            const res = await axios.post(`http://127.0.0.1:8000/api/clases/`, this.clase)
            if (res.status == 200) {
                this.$router.push({ name: 'Clases' })
                Swal.fire({
                    position: 'top-end',
